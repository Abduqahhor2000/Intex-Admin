        ...
        <>
            <div className="px-10 py-6">
                <div className="flex justify-end items-center">
                    <div data-testid="add_category" onClick={()=>{setCategoryMadal(true); setId(null)}} className="h-16 rounded-3xl py-4 px-3 text-center text-xl font-base text-white shadow-category-btn shadow-gray-400 cursor-pointer bg-blue-green">
                        + Добавить категории 
                    </div>
                </div>
                    ...
                    <tbody>
                        {
                            categories.map(item => {
                                return (
                                    <tr key={item.category_id} data-testid={`category_item_${item.category_id}`} className="bg-white">
                                        ...
                                        <td className="py-4 rounded-r-3xl pr-11">
                                            <div className="flex justify-end">
                                                <span data-testid={`category_edit_${item.category_id}`} onClick={() =>{ setCategoryMadal(true); setName_ru(item.name_ru); setName_uz(item.name_uz); setId(item.category_id)}} className="p-2 cursor-pointer">
                                                ...
                                                <span data-testid={`category_del_${item.category_id}`} onClick={() =>{ setCategoryMadal("del"); setId(item.category_id)} } className="p-2 ml-2 cursor-pointer">
