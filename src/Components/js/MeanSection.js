...
import {
  addAllConsultations,
  delAllConsultations,
} from "../../redux/consultationReducer";
...
import {
  addProductStatus,
  delProductStatus,
} from "../../redux/productStatusReducer";
    ...
    <>
      <div className="flex">
        <div className="w-72 h-screen bg-white pl-10 fixed">
          <div className="text-blue-green text-2xl py-8 mb-9 font-semibold">
            <a href="/">INTEX-SHOP.UZ</a>
          </div>
          <ul className="flex flex-col text-gray-640">
            <Link
              ...
              className={`mb-4 text-2xl cursor-pointer hover:text-blue-green-300 ${
                pathname === "/products"
                  ? "hover:text-blue-green text-blue-green cursor-text"
                  : ""
              }`}
            >
              Продукты
        ...
        <div className="bg-lighter w-header min-h-screen ml-72 pt-24">
          <div className="fixed z-10 bg-mean-lite flex items-center text-2xl justify-end w-header px-11 top-0 right-0 h-24 after:content-[''] after:w-screen after:bottom-0 after:right-0 after:absolute after:bg-line after:h-0.5">
            <div className="cursor-pointer after:block text-lite-brouwn hover:text-blue-green-300">
              <a href="https://next.intex-shop.uz">Просмотр веб-сайта</a>
            </div>
            ...
            <div className="flex text-lite-brouwn">
              <svg
                className="mr-2 mt-1"
                xmlns="http://www.w3.org/2000/svg"
                width="20"
              ...
              <span
                onClick={() => {
                  dispatch(removeUser());
                  dispatch(delAllCategories());
                  dispatch(delAllConsultations());
                  dispatch(delAllOrders());
                  dispatch(delAllProducts());
                  dispatch(delProductStatus());
                  dispatch(delAllSiteInfo());
                }}
                className="hover:text-blue-green-300 cursor-pointer mt-1.5 ml-5"
              >
                <ImExit />
              </span>
            </div>
