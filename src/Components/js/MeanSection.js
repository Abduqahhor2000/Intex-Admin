...
export default function MeanSection() {
    ...
    return(
        <>
            <div className="flex">
                <div className="w-72 h-screen bg-white pl-10 fixed">
                    <div className="text-blue-green text-2xl py-8 mb-9 font-semibold"><a href="/">INTEX-SHOP.UZ</a></div>
                    <ul className="flex flex-col text-gray-640">
                        <Link to="/products" className={`mb-4 text-2xl cursor-pointer hover:text-blue-green-300 ${pathname === "/products" ? "hover:text-blue-green text-blue-green cursor-text" : "" }`}>Продукты</Link>
                        <Link to="/orders" className={`mb-4 text-2xl cursor-pointer hover:text-blue-green-300 ${pathname === "/orders" ? "hover:text-blue-green text-blue-green cursor-text" : "" }`}>Заказы</Link>
                        <Link to="/types" className={`mb-4 text-2xl cursor-pointer hover:text-blue-green-300 ${pathname === "/types" ? "hover:text-blue-green text-blue-green cursor-text" : "" }`}>Категории</Link>
                        <Link to="/site" className={`mb-4 text-2xl cursor-pointer hover:text-blue-green-300 ${pathname === "/site" ? "hover:text-blue-green text-blue-green cursor-text" : "" }`}>Сайт</Link>
                    </ul>    
                </div>
                <div className="bg-lighter w-header min-h-screen ml-72 pt-24">
                    <div className="fixed z-10 bg-mean-lite flex items-center text-2xl justify-end w-header px-11 top-0 right-0 h-24 after:content-[''] after:w-screen after:bottom-0 after:right-0 after:absolute after:bg-line after:h-0.5">
                        <div className="cursor-pointer after:block text-lite-brouwn hover:text-blue-green-300"><a href="https://intex-shop.uz">Просмотр веб-сайта</a></div>
                        <div className="mx-7 text-lite-brouwn hover:text-blue-green-300">
                        ...
                        <div onClick={() =>{ dispatch(removeUser()); dispatch(delAllCategories()); dispatch(delAllConsultations()); dispatch(delAllOrders()); dispatch(delAllProducts()); dispatch(delProductStatus()); dispatch(delAllSiteInfo())}} className="hover:text-blue-green-300 flex cursor-pointer text-lite-brouwn hover:text-blue-green-300">
