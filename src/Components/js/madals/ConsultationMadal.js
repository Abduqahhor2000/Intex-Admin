...
export default function ConsultationMadal({consulID, setConsulID}) {
    ...
    return (
        <>
            <div onClick={()=>{ setConsulID("")}} className="fixed z-10 top-0 left-0 w-screen h-screen" style={{"backgroundColor": "rgba(0, 0, 0, 0.2)", "backdropFilter": "blur(7px)"}}></div>
            <div className="fixed z-10 flex flex-col justify-between items-center w-2/3 h-96 bottom-1/2 right-1/2 translate-x-1/2 translate-y-1/2 bg-slate-100 p-12 pt-6 rounded-3xl">
                <span onClick={() =>{ setConsulID("")}} className="absolute top-8 right-8 p-2 rounded-full hover:bg-slate-200 cursor-pointer">
                ...
                </span>
                {
                    ...
                    <>
                        ...
                        <div className="flex justify-around w-full">
                            {
                                reqStatus === true ? <span className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-3" style={{"backgroundColor" : "rgba(0, 147, 152, 1)"}}>
                                                   ...
                                                   : <span onClick={()=> {deleteConsul()}} className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-1.5" style={{"backgroundColor" : "rgba(0, 147, 152, 1)"}}>Да</span>
                            }
                            <span onClick={()=> setConsulID("")} className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-1.5" style={{"backgroundColor" : "rgb(152, 0, 147)"}}>Нет</span>   
                        </div>
                    </> :
