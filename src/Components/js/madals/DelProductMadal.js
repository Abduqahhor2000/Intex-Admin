...
export default function DelProductMadal({productID, setProductID}) {
    ...
    return (
        <>
            <div onClick={()=>{ setProductID("");}} className="fixed z-10 top-0 left-0 w-screen h-screen bg-black-02 backdrop-blur"></div>
            <div data-testid="thisisdelmodal" className="fixed z-10 flex flex-col justify-between items-center w-2/3 h-96 bottom-1/2 right-1/2 translate-x-1/2 translate-y-1/2 bg-slate-100 p-12 pt-6 rounded-3xl">
                <span data-testid="exit_delmodal" onClick={() =>{ setProductID("");}} className="absolute top-8 right-8 p-2 rounded-full hover:bg-slate-200 cursor-pointer">
                ...
                </span>
                {
                    ...
                    <>
                        ...
                        <div className="flex justify-around w-full">
                            {
                                reqStatus !== true ? <span onClick={()=> {deleteProduct()}} className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-1.5 bg-blue-green">Да</span>
                                                   : <span className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-3.5 bg-blue-green">
                            ...
                            <span onClick={()=>{ setProductID("");}} className="block cursor-pointer h-12 rounded-3xl text-3xl text-white px-16 py-1.5 bg-red-blue">Нет</span>   
                        </div>
                    </> :
